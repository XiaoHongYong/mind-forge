 ...
 * enforces.
 *
 * The native `.mmforge` format is the only one expected to be lossless
...
import { treeToMmforge, mmforgeToTree } from '../mmforgeFormat';
...
describe('round-trip fidelity', () => {
  expectRoundTrip('native .mmforge', NATIVE,
    (r) => treeToMmforge(r),
    (d, t) => mmforgeToTree(d as string, t));
 ...
 * (docs/freemind-mm-format-spec.md). These assert the *bytes*, not just that
 ...
 * (docs/wisemapping-format-spec.md). These assert the bytes real WiseMapping
