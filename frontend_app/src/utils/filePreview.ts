...
function renderCardPreviewSvg(file: File): Uint8Array {
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="200" viewBox="0 0 320 200">
      ...
      <text x="34" y="156" font-family="ui-sans-serif, system-ui, sans-serif" font-size="11" fill="#cbd5e1">File preview card</text>
    </svg>
  `.trim();
 ...
 *
 * Unlike `createFilePreview` this is a glyph, not a card: no
...
export async function createFilePreview(file: File): Promise<{
  bytes: Uint8Array;
  contentType: string;
  kind: 'image' | 'card';
}> {
