...
| 鸿蒙（ArkWeb） | 选择器返回授权 URI + ArkTS 桥的 `readFile` / `writeFile` |
| 浏览器 | 无文件系统：`<input type=file>` 导入、下载导出 |
...
桌面端把绝对路径永久记住，随便重开。**鸿蒙是沙箱的**：
选择器返回一个 `file://` URI，**系统在应用退出时收回授权**。

因此「最近文件」能否重开取决于设备是否支持授权持久化
（syscap + `FILE_ACCESS_PERSIST`；手机上常见不支持，错误码 801）。
前端通过异步能力探测决定是否显示该入口，**默认关闭**——
宁可少一个按钮，也不给一个必然失败的按钮。

会话存储的 `path` 就是那个 URI（保持不透明，作为备份 / 最近文件的键）；
只有展示用的标题与格式从解码后的形式推导。详见
[`harmonyos-architecture.md`](harmonyos-architecture.md) 第三节。
...
鸿蒙壳多一层**由系统强制**的保证：应用**不申请** `ohos.permission.INTERNET`，
资源全部从内存提供。一个逃过拦截的请求会硬失败，而不是悄悄把文档发出去。
