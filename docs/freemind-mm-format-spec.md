...
## 7. Round-trip implications for MindForge
...
> **Password-protected FreeMind/FreePlane branches are out of scope.** Those
> apps store children in `ENCRYPTED_CONTENT`. MindForge does not decrypt them —
> the password is not in the file. Such nodes import with a
> `🔒 Encrypted branch` note so the locked content is visible, not silently
> dropped. See `freeplane-mm-format-spec.md` §8 for the algorithm details.
