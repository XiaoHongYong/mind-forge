...
/** A link from a node to another local mind-map file. */
export interface NodeLink {
  type: 'file';
  /** Absolute path of the linked document. */
  path: string;
  /**
   * Display title when the link was made. Kept on the node so the strip
   * renders without resolving the path (which may have moved or been deleted).
   */
  label?: string;
}
...
export interface NodeParts {
  ...
  link: { id: string; path: string; label: string } | null;
  hasCheckbox: boolean;
  hasProgress: boolean;
  hasNote: boolean;
