...
describe('measureNodeSize', () => {
  ...
  it('adds a strip for a vault link, above the URLs', () => {
    ...
    const link = { type: 'file' as const, path: '/tmp/v1.mmforge', label: 'Roadmap' };
  ...
  it('widens for the link label, not for its id', () => {
    ...
    const narrow = measure({ text: 'x', link: { type: 'file' as const, path: '/tmp/v1.mmforge', label: 'R' } });
    const wide = measure({ text: 'x', link: { type: 'file' as const, path: '/tmp/v1.mmforge', label: 'a'.repeat(40) } });
    expect(wide.w).toBeGreaterThan(narrow.w);
  });

  it('falls back to the path when a link has no label', () => {
    const parts = layout.describeNode(node({ link: { type: 'file' as const, path: '/tmp/v1.mmforge' } }));
    expect(parts.link).toEqual({ id: '/tmp/v1.mmforge', path: '/tmp/v1.mmforge', label: '/tmp/v1.mmforge' });
  });
